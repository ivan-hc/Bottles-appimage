...
DEPENDENCES="7zip alsa-lib ibus libibus libpulse nss-mdns pipewire procps-ng xterm $lib32_pkgs" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
libxmlb.so libmp3lame libnss_mdns libproxy libstemmer.so libvorbis ordlookup pefile urllib vkbasalt $lib_audio_keywords $lib_browser_launcher"
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (hicolor-icon-theme|xterm|xapp|python|python-gobject|python-pycurl|procps-ng|patool|p7zip|imagemagick|icoextract|cabextract|7zip) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}

...
wayland \
xorg-apps xorg-server xorg-xwayland xterm" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCES ARE NEEDED
...
BINSAVED="patool png py svg wine xz"
SHARESAVED="adwaita appstream gnome gtk icons themes vk xml"
...
LIBSAVED="adwaita appstream cairo d3d decor gdk-pixbuf GL gl gnome gtk libdav libgraphene librsvg lzo module pau python repository selinux stemmer xml png svg $lib_audio_keywords $lib_browser_launcher"
...
_determine_packages_and_libraries() {
	...
	elif [ "$arg" != wine ] && [ "$arg" != autoconf ] && [ "$arg" != autoconf ] && [ "$arg" != automake ] && [ "$arg" != bison ] && [ "$arg" != debugedit ] && [ "$arg" != dkms ] && [ "$arg" != fakeroot ] && [ "$arg" != flatpak ] && [ "$arg" != linux ] && [ "$arg" != gcc ] && [ "$arg" != make ] && [ "$arg" != pacman ] && [ "$arg" != patch ] && [ "$arg" != systemd ]; then
		_extract_package
...
_remove_more_bloatwares() {
	...
	lib_remove="gcc libgphobos.so libjavascript libwebkit perl systemd"
	for r in $lib_remove; do
		rm -Rf ./"$APP".AppDir/.junest/usr/lib/"$r"*
	done
	share_remove="gcc autoconf automake fonts gir-1.0 git info pacman"
